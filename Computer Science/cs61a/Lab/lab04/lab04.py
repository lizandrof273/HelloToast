...
def if_this_not_that(i_list, this):
    ...
    #    print (i_list[n])
    #else:
    #    return if_this_not_that(i_list(n+1), this)
# City ADT
...
def distance(city1, city2):
    ...
    5.0
    """
    city1_lat, city1_lon,city2_lat, city2_lon= get_lat(city1), get_lon(city1),get_lat(city2),get_lon(city2)
    return sqrt((city1_lat - city2_lat)**2+ (city1_lon - city2_lon)**2)
    #(x1 - x2)**2 + (y1 - y2)**2
    #(x1, y1) and (x2, y2)
# Q5
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    city_1_distance = abs(get_lat(city1) - lat) + abs(get_lon(city1) - lon)
    city_2_distance = abs(get_lat(city2) - lat) + abs(get_lon(city2) - lon)
