...
def store_digits(n):
    ...
    if n <10:
        ...
        while 10**b < n:
            b+=1
        b-=1
        return Link(n//10**b, store_digits(n%10**b))
...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        child = t.branches[0]
        return is_bst(child) and (t.label >= bst_max(child) or t.label<bst_min(child))
# Q8
...
def in_order_traversal(t):
    ...
    if t.is_leaf():
        ...
        yield from in_order_traversal(left_branch)
        yield t.label
        yield from in_order_traversal(right_branch)
