...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place != hive:
            if place.bees:
                if self.min_range <= count and  count <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        if self.digesting == 0:
            ran_bee = random_or_none(self.place.bees)
            if ran_bee != None:
                self.eat_bee(ran_bee)
        else:
            self.digesting -=1
...
# END Problem 13
    ...
    def __init__(self, armor=1,place = None):
        # BEGIN Problem 13
        # if count = 0
        #     trueQueen = ScubaThrower.__init__(self, colony)
        #     count += 1
        self.ants_lst = []
        self.armor = armor
        if QueenAnt.count >= 1:
    ...
    def action(self, colony):
        ...
        while current_place != None:
            if current_place.ant:
                if not current_place.ant in self.ants_lst:
